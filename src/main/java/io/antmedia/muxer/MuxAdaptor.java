...
public class MuxAdaptor implements IRecordingListener, IEndpointStatusListener {
	...
	public boolean init(IScope scope, String streamId, boolean isAppend) {
		...
		if (hlsMuxingEnabled) {
			HLSMuxer hlsMuxer = new HLSMuxer(vertx, hlsListSize, hlsTime, hlsPlayListType, getAppSettings().getHlsFlags(), getAppSettings().isHlsEncryptionEnabled(), getAppSettings().getHlsEncryptionKeyInfoFile());
